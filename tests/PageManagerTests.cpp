...
uint8_t *Allocate(uint64_t size, uint32_t protection = PAGE_READWRITE) {
  ...
  void *raw = ::mmap(reinterpret_cast<void *>(test_address), size,
                     ToHostProt(protection), flags, -1, 0);
  Check(raw != MAP_FAILED, "low mmap failed");
  Check(reinterpret_cast<uintptr_t>(raw) + size <=
            Libs::Graphics::TRACKER_ADDRESS_SIZE,
        "mapping outside tracker address space");

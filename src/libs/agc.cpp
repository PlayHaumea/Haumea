	...
	KYTY_SYSV_ABI bool ReserveDW(uint32_t num_dw) {
		...
		if (num_dw > remaining) {
			if (callback == nullptr) {
				...
				const uint64_t callback_args[3] = {
				    reinterpret_cast<uint64_t>(this), num_dw + reserved_dw,
				    reinterpret_cast<uint64_t>(user_data)};
				uint64_t callback_result = 0;
				const bool result =
				    Loader::CallGuestVia(reinterpret_cast<uint64_t>(callback), callback_args, 3,
				                         &callback_result)
				        ? callback_result != 0
				        : callback(this, num_dw + reserved_dw, user_data);
			if (!result) {

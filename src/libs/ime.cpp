...
int KYTY_SYSV_ABI ImeUpdate(EventHandler handler) {
	...
	for (auto& item: dispatch) {
		...
		// The handler is the guest's own. Where the host CPU is not the guest CPU, calling it
		// directly executes x86-64 bytes as host code.
		const uint64_t handler_args[2] = {reinterpret_cast<uint64_t>(item.arg),
		                                  reinterpret_cast<uint64_t>(&item.queued.event)};
		uint64_t       guest_result    = 0;
		if (!Loader::CallGuestVia(reinterpret_cast<uint64_t>(handler), handler_args, 2,
		                          &guest_result)) {

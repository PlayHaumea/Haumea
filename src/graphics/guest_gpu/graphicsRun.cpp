...
void CommandProcessor::DrawIndirect(uint32_t data_offset, uint32_t draw_initiator, bool indexed) {
	...
	if (DrawAuditEnabled()) {
		const uint64_t addr = m_draw_indirect_args_base_addr + data_offset;
		const uint32_t words = indexed ? 5u : 4u;
		const bool     gpu   = m_renderer.GetBufferCache().IsRegionGpuModified(
            addr, static_cast<uint64_t>(words) * sizeof(uint32_t));
		uint32_t raw[5] {};
		std::memcpy(raw, args_addr, static_cast<size_t>(words) * sizeof(uint32_t));
		...
		if (n <= 64 || (n % 512) == 0) {
			std::printf("Haumea:DrawInd:Info: single indexed=%d gpu_args=%d addr=0x%016llx "
			            ...
			            static_cast<unsigned long long>(n), raw[0], raw[1], raw[2], raw[3], raw[4]);
			std::fflush(stdout);
		}
	}
...
void CommandProcessor::DrawIndirectMulti(uint32_t data_offset, uint32_t max_count_or_count,
	...
	if (DrawAuditEnabled()) {
		const uint64_t args_base = m_draw_indirect_args_base_addr + data_offset;
		const bool     gpu_count =
		    count_addr != nullptr &&
		    m_renderer.GetBufferCache().IsRegionGpuModified(
		        reinterpret_cast<uint64_t>(const_cast<const uint32_t*>(count_addr)), sizeof(uint32_t));
		const bool gpu_args = m_renderer.GetBufferCache().IsRegionGpuModified(
		    args_base, static_cast<uint64_t>(stride_in_bytes));
		static std::atomic<uint64_t> seen {0};
		const uint64_t n = seen.fetch_add(1, std::memory_order_relaxed) + 1;
		if (n <= 64 || (n % 512) == 0) {
...
void CommandProcessor::DispatchIndirect(uint32_t data_offset, uint32_t mode) {
	...
	CheckBuffer();
	CurrentBuffer().SetDebugInfo(
	    static_cast<uint32_t>(CommandBufferDebugOp::DispatchIndirect), m_submit_id,
	    args->thread_group_x, args->thread_group_y, args->thread_group_z, mode, args_addr);

	{
		static std::atomic<uint64_t> indirect_count {0};
		static std::atomic<uint64_t> indirect_zero {0};
		const uint64_t               seen = indirect_count.fetch_add(1, std::memory_order_relaxed);
		const uint64_t               zero =
		...
		if (seen < 16 || (seen % 4096) == 0) {
			std::printf("Haumea:Dispatch:Info: indirect seen=%llu zero=%llu groups=%ux%ux%u "
			            ...
			            static_cast<unsigned long long>(m_sh_ctx.GetCs().cs_regs.data_addr));
			std::fflush(stdout);
		}
	}

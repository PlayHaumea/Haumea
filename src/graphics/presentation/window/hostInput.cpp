...
void SetButton(uint32_t button, bool down) {
	constexpr int keyboard_port = 0;
	if (button == Controller::PAD_BUTTON_L2) {
		Controller::ControllerAxis(keyboard_port, Controller::HOST_INPUT_CONTROLLER_ID,
		                           Controller::Axis::TriggerLeft, down ? 255 : 0);
	} else if (button == Controller::PAD_BUTTON_R2) {
		Controller::ControllerAxis(keyboard_port, Controller::HOST_INPUT_CONTROLLER_ID,
		                           Controller::Axis::TriggerRight, down ? 255 : 0);
	} else if (button != 0) {
		Controller::ControllerButton(keyboard_port, Controller::HOST_INPUT_CONTROLLER_ID, button,
		                             down);
	}
}
...
void SetStickAxis(Controller::Axis axis, bool negative, bool positive) {
	...
	Controller::ControllerAxis(keyboard_port, Controller::HOST_INPUT_CONTROLLER_ID, axis, value);
}

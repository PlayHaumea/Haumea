...
constexpr const char* KYTY_METAL_SURFACE_EXTENSION_NAME = "VK_EXT_metal_surface";
...
using PfnCreateMetalSurface = VkResult (*)(VkInstance, const MetalSurfaceCreateInfo*,
                                           const VkAllocationCallbacks*, VkSurfaceKHR*);

static bool CreateSurface(SDL_Window* window, vk::Instance instance,
                          vk::SurfaceKHR::CType* surface) {
	void* layer = ExternalSurface();
	if (layer == nullptr) {
		return SDL_Vulkan_CreateSurface(window, static_cast<vk::Instance::CType>(instance),
		                                surface) == SDL_TRUE;
	}

	auto create = reinterpret_cast<PfnCreateMetalSurface>(
	    VULKAN_HPP_DEFAULT_DISPATCHER.vkGetInstanceProcAddr(
	        static_cast<vk::Instance::CType>(instance), "vkCreateMetalSurfaceEXT"));
	if (create == nullptr) {
		LOGF("vkCreateMetalSurfaceEXT is missing\n");
		return false;
	}

	MetalSurfaceCreateInfo info {};
	info.sType = static_cast<VkStructureType>(1000217000); // VK_STRUCTURE_TYPE_METAL_SURFACE_CREATE_INFO_EXT
	info.layer = layer;

	const VkResult result =
	    create(static_cast<vk::Instance::CType>(instance), &info, nullptr, surface);
	if (result != VK_SUCCESS) {
		LOGF("vkCreateMetalSurfaceEXT failed: %d\n", static_cast<int>(result));
		return false;
	}
	return true;
}
...
static void VulkanGetExtensions(SDL_Window* window, VulkanExtensions& r) {
	if (ExternalSurface() != nullptr) {
		r.required_extensions = {VK_KHR_SURFACE_EXTENSION_NAME, KYTY_METAL_SURFACE_EXTENSION_NAME};
	} else {
		...
		auto sdl_result =
		    SDL_Vulkan_GetInstanceExtensions(window, &required_extensions_count, nullptr);
...
void WindowContext::CreateVulkan() {
	...
	const int32_t use_metal_heap = 1;
	const VkLayerSettingEXT setting {
	    "MoltenVK", "MVK_CONFIG_USE_MTLHEAP", VK_LAYER_SETTING_TYPE_INT32_EXT, 1,
	    &use_metal_heap};
	const VkLayerSettingsCreateInfoEXT settings {
	    VK_STRUCTURE_TYPE_LAYER_SETTINGS_CREATE_INFO_EXT,
	    r.enable_validation_layers ? static_cast<const void*>(&dbg_create_info) : nullptr, 1,
	    &setting};
	inst_info.sType                   = vk::StructureType::eInstanceCreateInfo;
	inst_info.pNext                   = &settings;
	inst_info.flags =
	    (HasExtension(r.available_extensions, VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME)
	...
	if (!CreateSurface(window, graphic_ctx.instance, &native_surface)) {
...
#if defined(__APPLE__)
		if (HasExtension(available_extensions, "VK_EXT_metal_objects")) {
			device_extensions.push_back("VK_EXT_metal_objects");
		}
#endif
	}
...
void WindowContext::RecreateSurface() {
	EXIT_IF((window == nullptr && ExternalSurface() == nullptr) || graphic_ctx.instance == nullptr);
	if (surface != nullptr) {

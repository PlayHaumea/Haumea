...
struct PageManager::Impl {
...
#elif defined(__APPLE__)
		// 4 KB under Rosetta, 16 KB on Apple silicon; TRACKER_PAGE_SIZE follows the host.
	...
	uint32_t WatcherState(uint64_t vaddr) const noexcept {
		auto* region = FindRegion(vaddr);
		if (region == nullptr) {
			return 0;
		}
		SpinGuard lock(region->lock);
		const auto page = (vaddr % REGION_SIZE) / PAGE_SIZE;
		const auto& state = region->pages[page];
		return static_cast<uint32_t>(state.write_watchers) |
		       static_cast<uint32_t>(state.access_watchers) << 8u |
		       state.Perms() << 16u | static_cast<uint32_t>(state.managed) << 24u;
	}

	bool RestoreProtection(uint64_t vaddr) noexcept {
		auto* region = FindRegion(vaddr);
		if (region == nullptr) {
			return false;
		}
		...
		Protect(PageStart(vaddr), PAGE_SIZE, state.Perms());
		return true;
	}

	bool NoteFault(uint64_t vaddr) noexcept {
		auto* region = FindRegion(vaddr);
		if (region == nullptr) {
			return false;
		}
		SpinGuard lock(region->lock);
		auto& state = region->pages[(vaddr % REGION_SIZE) / PAGE_SIZE];
		if (!state.managed) {
			return false;
		}
		if (state.faults != UINT8_MAX) {
			++state.faults;
		}
		if (g_hot_page_escape && state.faults == 3) {
			Protect(PageStart(vaddr), PAGE_SIZE, READ_WRITE_PROTECTION);
		}
		return g_hot_page_escape && state.faults >= 3;
	}

	bool HasHotPage(uint64_t vaddr, uint64_t size) const noexcept {
		if (!g_hot_page_escape) {
			return false;
		}
		if (vaddr == 0 || size == 0 || vaddr >= ADDRESS_SIZE || size > ADDRESS_SIZE - vaddr) {
			return false;
		...
		for (auto page = begin; page < end; page += PAGE_SIZE) {
			auto* region = FindRegion(page);
			if (region == nullptr) {
				continue;
			}
			SpinGuard lock(region->lock);
			if (region->pages[(page % REGION_SIZE) / PAGE_SIZE].faults >= 3) {
				return true;
			}
		}
		return false;
...
uint32_t PageManager::WatcherState(uint64_t vaddr) const noexcept {
	return m_impl->WatcherState(vaddr);
}

bool PageManager::RestoreProtection(uint64_t vaddr) noexcept {
	return m_impl->RestoreProtection(vaddr);
}

bool PageManager::NoteFault(uint64_t vaddr) noexcept {
	return m_impl->NoteFault(vaddr);
}

bool PageManager::HasHotPage(uint64_t vaddr, uint64_t size) const noexcept {
	return m_impl->HasHotPage(vaddr, size);
}

...
constexpr uint64_t     PipelineCacheSaveInterval = 4;
std::atomic<uint64_t>  g_pipeline_saved_count {0};

void RecordPipelineCreate(const GraphicContext& graphics,
                          std::chrono::steady_clock::time_point start) {
	const auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(
	                         std::chrono::steady_clock::now() - start)
	                         .count();
	const uint64_t created = g_pipeline_create_count.fetch_add(1, std::memory_order_relaxed) + 1;
	g_pipeline_create_microseconds.fetch_add(static_cast<uint64_t>(elapsed),
	                                         std::memory_order_relaxed);

	uint64_t saved = g_pipeline_saved_count.load(std::memory_order_relaxed);
	if (created - saved >= PipelineCacheSaveInterval &&
	    g_pipeline_saved_count.compare_exchange_strong(saved, created,
	                                                   std::memory_order_relaxed)) {
		graphics.SavePipelineCache();
	}
}
...
PipelineCache::GraphicsPipeline& PipelineCache::CreateGraphicsPipeline(
	...
	RecordPipelineCreate(m_graphics, pipeline_start);
...
PipelineCache::CreateComputePipeline(ShaderComputeInputInfo&      input_info,
...

	{
		...
		uint64_t code_hash = 0xcbf29ce484222325ull;
		for (const auto word: cs_spirv) {
...
uint64_t PipelineGetCreateCount() {
	return g_pipeline_create_count.load(std::memory_order_relaxed);
}

uint64_t PipelineGetCreateMicroseconds() {
	return g_pipeline_create_microseconds.load(std::memory_order_relaxed);
}
} // namespace Libs::Graphics

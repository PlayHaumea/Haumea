...
static bool ConsumeMetadataColorOperation(const RenderCommandBuffer& buffer) {
	...
	// Expanded Vulkan images are stored rather than compressed guest surfaces, so no equivalent
...
static void SetDrawDebugPhase(RenderCommandBuffer& buffer, uint64_t submit_id,
	...
	buffer.SetDrawPhase(static_cast<uint32_t>(draw.debug_op), submit_id, phase, draw.index_count,

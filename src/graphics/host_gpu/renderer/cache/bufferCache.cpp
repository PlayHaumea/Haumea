...
constexpr uint64_t StagingBufferSize = 64 * MiB;
constexpr uint64_t StreamBufferSize  = 8 * MiB;
constexpr uint64_t DownloadBufferSize = 4 * MiB;
constexpr uint64_t DeviceBufferSize  = 16 * MiB;
#else
constexpr uint64_t StagingBufferSize = 512 * MiB;
constexpr uint64_t StreamBufferSize  = 64 * MiB;
constexpr uint64_t DownloadBufferSize = 32 * MiB;
constexpr uint64_t DeviceBufferSize  = 128 * MiB;
#endif
...
BufferCache::BufferCache(GraphicContext& graphics, CommandScheduler& scheduler,
    ...
    : m_graphics(graphics), m_scheduler(scheduler),
      m_gds_buffer(graphics, scheduler, MemoryUsage::Stream, 0, AllFlags, GdsBufferSize),
	      ...
	      m_staging_buffer(graphics, scheduler, MemoryUsage::Upload, StagingBufferSize),
	      m_stream_buffer(graphics, scheduler, MemoryUsage::Stream, StreamBufferSize),
	      m_download_buffer(graphics, scheduler, MemoryUsage::Download, DownloadBufferSize),
	      m_device_buffer(graphics, scheduler, MemoryUsage::DeviceLocal, DeviceBufferSize),
...
ImageBufferSource BufferCache::ObtainBufferForImage(uint64_t vaddr, uint64_t size) {
	...
	auto [staging, stage_offset] = m_staging_buffer.Map(size, 16, true);
	if (staging == nullptr) {
		EXIT("BufferCache: image staging allocation failed, addr=0x%016" PRIx64
		     " size=0x%016" PRIx64 " staging_capacity=0x%016" PRIx64 "\n",
		     vaddr, size, m_staging_buffer.Size());
	}
	const bool direct_backing = Libs::LibKernel::Memory::TryReadBacking(vaddr, staging, size);
	const bool prt_backing =
	    !direct_backing && Libs::LibKernel::Memory::TryReadPrtBacking(vaddr, staging, size);
	const bool mapped_memory = !direct_backing && !prt_backing &&
	                           Libs::LibKernel::Memory::TryReadMappedMemory(vaddr, staging, size);
	if (!direct_backing && !prt_backing && !mapped_memory) {
		const auto mapped_size  = Libs::LibKernel::Memory::ClampRangeSize(vaddr, size);
		const auto span_details = Libs::LibKernel::Memory::DescribeSpan(vaddr, size);
		EXIT("BufferCache: failed to read mapped guest image backing, addr=0x%016" PRIx64
		     " size=0x%016" PRIx64 " mapped=0x%016" PRIx64 "\n\t %s\n",
		     vaddr, size, mapped_size, span_details.c_str());
	}
